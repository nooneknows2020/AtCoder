...
- for文は「iが1ずつ増えながらN回処理を繰り返す機能」と考える
- ループを使わずにプログラムを書いてみてから、ループで書き直す方法が有効
- 処理が1文のみの場合、`{}`を省略可能

#### for文とwhile文の違い

- カウンタ変数のスコープ
  - for文: カウンタ変数のスコープがfor文の中に限定される
  - while文: カウンタ変数のスコープがwhile文の外にも及ぶ
...
- for文: continueの後、更新処理(i++など)が実行される
- while文: continueの後、条件式の判定に戻る(更新処理が飛ばされる可能性がある)
...
- 省略形
  - 初期化や更新を省略できる：`for (; 条件式;) { 処理 }`
- 無限ループ
  - すべてを省略して無限ループを作成：`for (;;) { 処理 }`
- 複数の変数の使用
